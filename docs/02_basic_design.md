...
| メッセージング | LINE Messaging API | `@line/bot-sdk` でFlex Message送信 |
...
バリデーション: `title`, `date`, `time`, `location` は必須。`description` は任意（デフォルト空文字列）。
...
Flex Messageを環境変数`LINE_GROUP_ID`で指定されたグループにpush送信する。メッセージにはLIFFアプリへのリンクボタンを含める。
...
LIFFアプリは1ページ構成で、タブで切り替える。

| タブ | 表示条件 | 内容 |
|------|---------|------|
| 練習一覧（メンバー向け） | 全ユーザー | 練習カード一覧 + 参加/不参加ボタン |
| 管理（管理者向け） | 管理者のみ表示 | 練習作成フォーム + 出欠集計 + LINE送信ボタン |
...
- 管理者専用エンドポイント（POST /api/practices, POST /api/practices/:id/announce）にミドルウェアを適用
...
| LINE_CHANNEL_ACCESS_TOKEN | Messaging APIのアクセストークン | (長い文字列) |
| LINE_GROUP_ID | 案内送信先のグループID | C1234567890abcdef |
